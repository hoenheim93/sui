...
type PublishResult = Vec<(Option<Symbol>, CompiledModule)>;
...
impl<'a> MoveTestAdapter<'a> for SuiTestAdapter<'a> {
    ...
    ) -> anyhow::Result<(Option<String>, PublishResult)> {
        self.next_task();
        let SuiPublishArgs {
            sender,
            upgradeable,
        ...
        match command {
            ...
            SuiSubcommand::UpgradePackage(UpgradePackageCommand {
                ...
                *state.named_address_mapping.get_mut(&package).unwrap() =
                ...
                let (output, mut modules) = self.upgrade_package(
                    package_id,
                    modules,
                    upgrade_capability,
                    dependencies,
...
impl<'a> SuiTestAdapter<'a> {
    fn upgrade_package(
        ...
        println!("Debug -- {}", upgrade_capability);
        ...
        Ok((output, vec![] /* TODO: return upgraded objects */))
    }
